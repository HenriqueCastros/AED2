...
void selecao(Musica *a, int n, int index, int* countComp, int* countMov){
    if (index != n){  
        int menor = index;
        for (int j = (index + 1); j < n; j++){
            if (strcmp(a[menor].name, a[j].name) > 0){
                menor = j;
                *countComp = *countComp + 1;
            }
        }

        swap(&a[menor], &a[index]);
        *countMov = *countMov + 3;
        selecao(a, n, index + 1, countComp, countMov);
    }
}

void insercaoPorCor(Musica *array, int n, int cor, int h, int* countComp, int* countMov){
    for (int i = (h + cor); i < n; i+=h) {
        Musica tmp = array[i];
        int j = i - h;
        while ((j >= 0) && (strcmp(array[j].id, tmp.id) > 0)) {
            array[j + h] = array[j];
            *countComp = *countComp + 1;
            *countMov = *countMov + 1;
            j-=h;
...
void shellsort(Musica *array, int n, int* countComp, int* countMov) {
    int h = 1;
    ...
    do {
        ...
        for(int cor = 0; cor < h; cor++){
            insercaoPorCor(array, n, cor, h, countComp, countMov);
        }
    } while (h != 1);
}
...
int main ( int argc, char* argv [ ] )
{
    ...
    inicio = clock();
    shellsort(mySongs, len, &countComp, &countMov);
    fim = clock();
