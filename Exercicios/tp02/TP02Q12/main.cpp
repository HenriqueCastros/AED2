...
void bolha(Musica *array, int n, int* countComp, int* countMov){
    int i, j;
    for (i = (n - 1); i > 0; i--) {
        for (j = 0; j < i; j++) {
            if (array[j].danceability > array[j + 1].danceability) {
...
int main ( int argc, char* argv [ ] )
{
    ...
    bolha(mySongs, len, &countComp, &countMov);
    for (int j = 0; j < len; j++)
    {
        int menor = j;
        for (int k = j; k <= len-1 && (mySongs[k].danceability == mySongs[j].danceability); k++) {
