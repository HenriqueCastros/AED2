...
public class Main {
    ...
    */
    ...
    public static void main(String[] args) throws Exception{
        ...
        for (int k = 0; k < 10; k++) {
            int menor = k;
            for (int j = (k + 1); j < entrada.length; j++){
                if (entrada[menor].getNome().compareTo(entrada[j].getNome()) > 0){
                    menor = j;
                    countComp++;
                }
            }
            swap(menor, k, entrada);
            countMov +=3;
        }
        ...
        FileWriter myWriter = new FileWriter("matrícula_sequencialParcial.txt");
