...
public class Main {
    ...
    */
    ...
    public static Musica[] countingSort(Musica[] array, int n) {
        ...
        for (int i = 0; i < n; count[array[i].getPopularity()]++, i++){
            countMov++;
        }
        ...
        //Ordenando
        for(int i = n-1; i >= 0; ordenado[count[array[i].getPopularity()]-1] = array[i], count[array[i].getPopularity()]--, i--){
            countMov++;
        }
        ...
        array = comparePop(array);
        return array;
    }
    
    public static Musica[] comparePop(Musica[] array){
        Musica[] resp = new Musica[array.length];
        for(int i = 0; i < array.length-1; i++){
            int menor = i;
            for (int j = i; j <= array.length-1 && (array[j].getPopularity() == array[i].getPopularity()); j++) {
    ...
    public static int getMaior(Musica[] array, int n) {
        int maior = array[0].getPopularity();

            for (int i = 0; i < n; i++) {
                if(maior < array[i].getPopularity()){
                    countComp++;
                    maior = array[i].getPopularity();
                }
            }
        return maior;	
    }
    ...
    public static void main(String[] args) throws Exception{
        ...
        countingSort(entrada, entrada.length);

        for(int k = 0; k< entrada.length; k++){
            System.out.println(entrada[k].print());
        }

        FileWriter myWriter = new FileWriter("matrícula_coutingsort.txt");
