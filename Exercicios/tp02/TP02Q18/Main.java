...
public class Main {
    ...
    */
    ...
    public static void compareDuration(Musica[] array){
        //Musica[] resp = new Musica[array.length];
        for(int i = 0; i < array.length-1; i++){
            int menor = i;
            for (int j = i; j <= array.length-1 && (array[j].getDuration() == array[i].getDuration()); j++) {
    ...
    public static void quicksort( Musica[] array, int esq, int dir) {
        int i = esq, j = dir;
        Musica pivo = array[(dir+esq)/2];
        while (i <= j) {
            while (array[i].getDuration() < pivo.getDuration()){
               i++;
               countComp++; 
            } 
            while (array[j].getDuration() > pivo.getDuration()){
                j--;
                countComp++;
            } 
            if (i <= j) {
