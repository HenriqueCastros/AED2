...
int getMaior(Musica *array, int n) {
    int maior = array[0].duration_ms;

    for (int i = 0; i < n; i++) {
        if(maior < array[i].duration_ms){
            maior = array[i].duration_ms;
        }
    }
    return maior;
}

void countingsort(Musica *array, int n, int* countComp, int* countMov) {
    ...
    for (int i = 0; i < n; count[array[i].duration_ms]++, i++);

    for(int i = 1; i < tamCount; count[i] += count[i-1], i++);

    for(int i = n-1; i >= 0; ordenado[count[array[i].duration_ms]-1] = array[i], count[array[i].duration_ms]--, i--);
...
int main ( int argc, char* argv [ ] )
{
    ...
    countingsort(mySongs, len, &countComp, &countMov);
    for (int j = 0; j < len; j++)
    {
        int menor = j;
        for (int k = j; k <= len-1 && (mySongs[k].duration_ms == mySongs[j].duration_ms); k++) {
