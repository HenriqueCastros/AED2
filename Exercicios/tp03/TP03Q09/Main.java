...
class CCelulaDup {
	...
	public CCelulaDup(Musica valorItem) {
		item = valorItem;
		ant = null;
		prox = null;
	}
	public CCelulaDup(Musica valorItem, CCelulaDup celulaAnt, CCelulaDup proxCelula) {
		item = valorItem;
		ant = celulaAnt;
		prox = proxCelula;
	}
...
private CCelulaDup primeira; // Referencia a primeira celula da lista (celula cabeca)
	...
	public void insereFim(Musica valorItem) {
		ultima.prox = new CCelulaDup(valorItem, ultima, null);
		ultima = ultima.prox;
		qtde++;
	}

	public void insereComeco(Musica valorItem) {
    ...
    public void insereIndice(Musica valorItem,int posicao) {
        if (posicao == 1 && (primeira != ultima)){
            this.insereComeco(valorItem);
        }else if(posicao == qtde+1 && (primeira != ultima)){
            this.insereFim(valorItem);
        }else if ((posicao > 1) && (posicao <= qtde) && (primeira != ultima)) {
			CCelulaDup aux = primeira;
			// Procura a posicao a ser inserido
			for (int i = 0; i < posicao; i++, aux = aux.prox);
            CCelulaDup tmp = new CCelulaDup(valorItem, aux, aux.prox);
	...
	public void imprime() {
		...
		while (aux != null) {
			System.out.println(aux.item.print());
			//System.out.print(" "+ aux.item.getDuration());
            aux = aux.prox;
		}
       // System.out.println("");
	}
	public boolean contemFor(Musica elemento) {
		boolean achou = false;
		for (CCelulaDup aux = primeira.prox; aux != null && !achou; aux = aux.prox)
			achou = aux.item.getId().equals(elemento.getId());
		return achou;
	}
    ...
    public Object retornaIndiceCel(int posicao) {
		if ((posicao >= 1) && (posicao <= qtde) && (primeira != ultima)) {
			CCelulaDup aux = primeira.prox;
			// Procura a posicao a ser inserido
			for (int i = 1; i < posicao; i++, aux = aux.prox)
	...
	public void remove(Musica valorItem) {
		if (primeira != ultima) {
			CCelulaDup aux = primeira.prox;
			boolean achou = false;
			while (aux != null && !achou) {
				achou = aux.item.getId().equals(valorItem.getId());
				if (!achou)
					aux = aux.prox;
			}
...
public class Main {
    ...
    */
    ...
    public static void swap(CCelulaDup celI, CCelulaDup celJ, CListaDup l) {
        Musica aux = celI.item;
        celI.item = celJ.item;
        celJ.item = aux;
    }

    public static void quicksort( CListaDup l, int esq, int dir) {
        int i = esq, j = dir;
        CCelulaDup celI = (CCelulaDup) l.retornaIndiceCel(esq);
        CCelulaDup celJ = (CCelulaDup) l.retornaIndiceCel(dir);
        CCelulaDup pivoCel = (CCelulaDup) l.retornaIndiceCel((dir+esq)/2);
        int pivo = pivoCel.item.getDuration();
        while (i <= j) {
            while (celI.item.getDuration() < pivo){
               i++;
               celI = celI.prox;
            } 
            while (celJ.item.getDuration() > pivo){
                j--;
                celJ = celJ.ant;
            } 
            if (i <= j) {
    ...
    public static void main(String[] args) throws Exception{
        ...
        CListaDup MusicaFilo = new CListaDup();
        int i = 0;
        ...
        i = 0;
        while (!isFim(val[i]) ){
            MusicaFilo.insereComeco(lookForMusica(val[i], entrada));
            i++;
        }
        quicksort(MusicaFilo, 1, MusicaFilo.quantidade());
        MusicaFilo.imprime();
        br.close();
    }
