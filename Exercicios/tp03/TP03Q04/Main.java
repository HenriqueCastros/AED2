...
class CLista {
	...
	public boolean insereIndice(Musica valorItem, int posicao) {
	...
	public void imprime() {
        ...
        int i = 0;
		while (aux != null) {
			System.out.println("["+i+"] "+aux.item.print());
			aux = aux.prox;
            i++;
		}
        
	}
	public boolean contem(Musica elemento) {
		boolean achou = false;
		CCelula aux = primeira.prox;
...
public class Main {
    ...
    */
    public static void processComand(String s, CLista MusicaList, Musica[] bd){
        String [] val = s.split(" ");
        Object holder = null;
        Musica aux = new Musica();
        switch(val[0]){
            case "RI":
                holder = MusicaList.removeRetornaComeco();
                if(holder != null){
                    aux = (Musica) holder;
                ...
                break;
            case "R*":
                holder = MusicaList.removeRetornaIndice(Integer.parseInt(val[1]));
                if(holder != null){
                    aux =  (Musica) holder;
                    System.out.println("(R) "+aux.getNome());
                }
                break;
            case "RF":
                holder = MusicaList.removeRetornaFim();
                if(holder != null){
                    aux =  (Musica) holder;
                    System.out.println("(R) "+aux.getNome());
                }
            ...
            case "II":
                ...
                MusicaList.insereComeco(aux);
                break;
            case "IF":
                aux = lookForMusica(val[1], bd);
                MusicaList.insereFim(aux);
                break;
            case "I*":
                aux = lookForMusica(val[2], bd);
                MusicaList.insereIndice(aux, Integer.parseInt(val[1]));
                break;
            default:
                break;
        }
    ...
    public static void main(String[] args) throws Exception{
        ...
        i = 0;
        while (!isFim(val[i])){
            MusicaFilo.insereFim(lookForMusica(val[i], entrada));
            i++;
        }
