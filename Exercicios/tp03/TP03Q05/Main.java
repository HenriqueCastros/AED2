...
class CCelula {
	public Musica item;
	public CCelula prox;    	
    public CCelula(Musica valorItem, CCelula proxCelula)
    {
        item = valorItem;
        prox = proxCelula;
    }    			
    public CCelula(Musica valorItem)
    {
        item = valorItem;
        prox = null;
    }    			        	
...
class CPilha {
	...
	public void empilha(Musica valorItem) {
	...
	public boolean contem(Musica valorItem) {
		CCelula aux = topo;
		while (aux != null) {
	...
	public boolean contemFor(Musica valorItem) {
	...
	public void imprime() {
        int i = 0;
		for (CCelula c = topo; c != null; c = c.prox, i++)
			System.out.println("["+i+"] "+c.item.print());
	}

    /*
        Metodo para imprimir fila do final ao comeco
     */
    public void imprimeFundoUp(){
        CPilha tmp = new CPilha();
        for (CCelula c = topo; c != null; c = c.prox)
			tmp.empilha((Musica)this.desempilha());
        tmp.imprime();
        for (CCelula c = tmp.topo; c != null; c = c.prox)
			this.empilha((Musica)tmp.desempilha());
    }
...
public class Main {
    ...
    */
    public static void processComand(String s, CPilha MusicaFilo, Musica[] bd){
        String [] val = s.split(" ");
        Object holder = null;
        Musica aux = new Musica();
        switch(val[0]){
            case "R":
                holder = MusicaFilo.desempilha();
                if(holder != null){
                    aux = (Musica) holder;
    ...
    public static void main(String[] args) throws Exception{
        ...
        i = 0;
        while (!isFim(val[i])){
            MusicaFilo.empilha(lookForMusica(val[i], entrada));
            i++;
        }

        i = Integer.valueOf(MyIO.readLine());
        for(int j = 0; j <i; j++){
            processComand( MyIO.readLine(), MusicaFilo, entrada);
        }
        MusicaFilo.imprimeFundoUp();
        br.close();
    }
